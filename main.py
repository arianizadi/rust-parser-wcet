...
def draw_and_export_graph(graph, labels, svg_path, cycles_list, cyclic_nodes=None, cyclic_edges=None):
    ...
    using demangled labels, and showing cycle counts under each node.
    If there is no cycle count for a node, displays 'Cycles: unknown'.
    """
    if not graph.nodes:
    ...
    try:
        ...
        # Draw cycle count labels under each node (if node is in a cycle)
        for node in node_list:
            ...
            # Compose the label
            if node in node_cycle_counts and node_cycle_counts[node] > 0:
                count = node_cycle_counts[node]
                label_text = f"Cycles: {count}"
            else:
                label_text = "Cycles: unknown"
...
def main():
    parser_arg = argparse.ArgumentParser(
        description="Analyze LLVM IR code (.ll), filter graph to user functions, detect cycles, demangle names, and export as SVG with cycle counts under each node."
    )
